    ...
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="card-header">
                <a href="{{route('frontend-menu.post.view')}}" class="btn btn-info btn-sm"><i class="fas fa-stream"></i> View Post sub</a>
            </div>
            <div class="card-body">
                <form action="{{ !empty($editData)? route('frontend-menu.post.update',$editData->id) : route('frontend-menu.post.store') }}" method="post" enctype="multipart/form-data" autocomplete="off" id="myForm">
                    ...
                    <div class="form-group col-sm-12">
                        ...
                        <select name="sub_menu_id" id="menu_id" class="form-control">
                            <option value="">Select Menu</option>
                                @foreach($sub_menu as $menu)
                                <option value="{{$menu->id}}">{{$menu->submenu_name}}</option>
                                @endforeach
                            </option>
                        </select>
                    </div>
