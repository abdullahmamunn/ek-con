    ...
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
              ...
              @php
                $menus = \App\MenuFrontend::get();  
                $submenus = \App\SubMenu::get();  
              @endphp
                    <div class="form-group col-sm-12">
                        <label>Menus</label>
                        <select name="menu_id" id="menu_id" class="form-control">
                            <option value="">Select Menu</option>
                                ...
                                <option value="{{$menu->id}}"{{($menu->id == $editData->menu_id ? 'selected' : '')}}>{{$menu->menu_name}}</option>
                                @endforeach
                            </option>
                        </select>
                    </div>
                    <div class="form-group col-sm-12">
                        ...
                        <select name="menu_id" id="menu_id" class="form-control">
                            <option value="">Select Menu</option>
                                @foreach($submenus as $s_menu)
                                <option value="{{$s_menu->id}}"{{($s_menu->id == $editData->submenu_id ? 'selected' : '')}}>{{$s_menu->submenu_name}}</option>
                                @endforeach
                            </option>
                        </select>
                    </div>
